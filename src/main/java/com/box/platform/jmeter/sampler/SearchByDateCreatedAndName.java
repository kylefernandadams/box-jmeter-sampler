...
/**
 *  JMeter sampler client which searches for Box files based on creation date range and name.
 */
public class SearchByDateCreatedAndName implements JavaSamplerClient {
    ...
    private static final String CREATED_FROM_DATE = "created.from.date";
    private static final String CREATED_TO_DATE = "created.to.date";
    ...
    private DateRange createDateRange = null;
    ...
    public Arguments getDefaultParameters() {
        ...
        defaultParameters.addArgument(CREATED_FROM_DATE, "${" + CREATED_FROM_DATE + "}");
        defaultParameters.addArgument(CREATED_TO_DATE, "${" + CREATED_TO_DATE + "}");
     ...
     *  - Get the created from and to dates
    ...
    public void setupTest(JavaSamplerContext setupContext) {
        try {
            ...
            String createdFromDateString = setupContext.getParameter(CREATED_FROM_DATE);
            Date createdFrom = dateFormat.parse(createdFromDateString);

            String createdToDateString = setupContext.getParameter(CREATED_TO_DATE);
            Date createdTo = dateFormat.parse(createdToDateString);
            createDateRange = new DateRange(createdFrom, createdTo);
    ...
    public SampleResult runTest(JavaSamplerContext runContext) {
        ...
        try{
            ...
            boxSearchParameters.setCreatedRange(createDateRange);
            long offset = 0;
