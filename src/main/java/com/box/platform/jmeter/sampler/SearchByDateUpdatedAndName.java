...
/**
 *  JMeter sampler client which searches for Box files based on modified date range and name.
 */
public class SearchByDateUpdatedAndName implements JavaSamplerClient {
    ...
    private static final String UPDATED_FROM_DATE = "updated.from.date";
    private static final String UPDATED_TO_DATE = "updated.to.date";
    ...
    private static final String DATE_TIME_FORMAT = "M-dd-yyyy hh:mm:ss";

    private DateRange updateDateRange = null;
    ...
    public Arguments getDefaultParameters() {
        ...
        defaultParameters.addArgument(UPDATED_FROM_DATE, "${" + UPDATED_FROM_DATE + "}");
        defaultParameters.addArgument(UPDATED_TO_DATE, "${" + UPDATED_TO_DATE + "}");
     ...
     *  - Get the modified from and to dates
    ...
    public void setupTest(JavaSamplerContext setupContext) {
        try {
            ...
            String updatedFromDateString = setupContext.getParameter(UPDATED_FROM_DATE);
            Date updatedFrom = dateFormat.parse(updatedFromDateString);

            String updatedToDateString = setupContext.getParameter(UPDATED_TO_DATE);
            Date updatedTo = dateFormat.parse(updatedToDateString);
            updateDateRange = new DateRange(updatedFrom, updatedTo);
    ...
    public SampleResult runTest(JavaSamplerContext runContext) {
        ...
        try{
            ...
            boxSearchParameters.setUpdatedRange(updateDateRange);
            long offset = 0;
